      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 lg:gap-6">
          {features.map((feature, index) => (
            ...
            >
              <div className="w-10 h-10 sm:w-12 sm:h-12 lg:w-14 lg:h-14 rounded-lg sm:rounded-xl bg-[#efb218] flex items-center justify-center mb-3 sm:mb-4 mx-auto group-hover:bg-[#efb218]/90 transition-all duration-300">
                {feature.customIcon ? (
                  <img src={dualCoreShield} alt="Shield" className="w-5 h-5 sm:w-6 sm:h-6 lg:w-8 lg:h-8 object-contain" />
                ) : (
                  feature.icon && <feature.icon className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-foreground group-hover:text-white transition-colors" />
                )}
              </div>
